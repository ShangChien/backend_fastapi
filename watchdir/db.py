import re, json, logging  # noqa: E401
	...
	with open(filePath,'r') as file:
		for line in file:
			if "Leave Link" in line :
				...
				strptime = datetime.strptime(time_str, "%a %b %d %H:%M:%S %Y")
				return strptime
	logging.info("%s: Timestrp data not found", filePath)
	...
	with open(filePath,'r') as file:
		...
		for n in range(len(lines)-1,0,-1):
			if 'Zero-point correction' in lines[n]:
				...
				return corrections
	logging.info("%s: Corrections data not found", filePath)
	...
	with open(filePath,'r') as file:
		for line in file:
			if target in line:
				...
				return crossing
	logging.info("%s: ElectronCrossing data not found", filePath)
...
def get_spectrum(filePath: str, target:str='Singlet') -> Union[ElectronCrossing,None]:
	...
	if energy==[] or oscillator == []:
		logging.info("%s: Spectrum data not found", filePath)
	...
	with open(filePath,'r') as f:
		for line in f:
			if "Exact polarizability" in line:
				...
				return polar
	logging.info("%s: Polar data not found", filePath)
...
def mol2smiles(mol:Any,filePath:str) -> Union[str,None]:
	try:
		...
		logging.info("%s: parse smiles failed",filePath)
...
def get_molData(filePath:str)	->	MolData:
	...
	print(moldata.model_dump())
	return moldata

def cur_process(moldata:MolData,cur:pg.Cursor[Any])->Any:
	name			= moldata.name_calc
	dirPath		=	moldata.dirPath.dict() # type: ignore
	fileType 	= [k for k in dirPath if dirPath[k] is not None][0]
	mol_without_none	=	{k: v for k, v in moldata.dict().items() if v is not None}
	...
	##已经存在则 updata data
		...
		if fileType=='ground':
			...
			cur.execute('UPDATE moldata.main SET corrections = corrections || %s WHERE name_calc = %s',
				[json.dumps(moldata.corrections.dict()),name]) #type:ignore
		elif fileType=='polar':
			cur.execute('UPDATE moldata.main SET dirPath = dirPath || %s WHERE name_calc = %s',
			...
			cur.execute('UPDATE moldata.main SET polar = polar || %s WHERE name_calc = %s',
				[json.dumps(moldata.polar.dict()),name]) #type:ignore
		elif fileType=='tdft':
			cur.execute('UPDATE moldata.main SET dirPath = dirPath || %s WHERE name_calc = %s',
				[json.dumps({'tdft':moldata.dirPath.tdft}),name]) #type:ignore
			cur.execute('UPDATE moldata.main SET tdft = tdft || %s WHERE name_calc = %s',
				[json.dumps(moldata.tdft.dict()),name]) #type:ignore
			cur.execute('UPDATE moldata.main SET spectrum = spectrum || %s WHERE name_calc = %s',
				[json.dumps({'uv_low':moldata.spectrum.uv_low.dict()}),name]) #type:ignore
		elif fileType=='uv':
			cur.execute('UPDATE moldata.main SET dirPath = dirPath || %s WHERE name_calc = %s',
				[json.dumps({'tdft':moldata.dirPath.uv}),name]) #type:ignore
			cur.execute('UPDATE moldata.main SET spectrum = spectrum || %s WHERE name_calc = %s',
				[json.dumps({'uv':moldata.spectrum.uv.dict()}),name]) #type:ignore
			cur.execute('UPDATE moldata.main SET tdft = tdft || %s WHERE name_calc = %s',
				[json.dumps({'s0_s1':moldata.tdft.s0_s1.dict()}),name]) #type:ignore
		elif fileType=='udft':
			cur.execute('UPDATE moldata.main SET dirPath = dirPath || %s WHERE name_calc = %s',
			...
			cur.execute('UPDATE moldata.main SET udft = udft || %s WHERE name_calc = %s',
				[json.dumps(moldata.udft.dict()),name]) #type:ignore
		else:
			logging.info('unknown,filetype:',fileType,dirPath)
			...
			for file_p in path_files:
				try:
					...
					try:
						...
						logging.info("%s: sql operate error", file_p)
				except Exception:
					logging.info("%s: parse moldata error", file_p)
...
if __name__ == "__main__":
	...
	logging.basicConfig(level=logging.INFO, 
                    ...
                    filename=logname,
                    filemode='a')
	logging.info("程序开始时间: %s", start_time.strftime('%Y-%m-%d %H:%M:%S'))
	dir_path = Path('W:/Data-month-statistics/202309/')
	i=0#统计次数
	loggin_str=get_login_str(file_p='.access.json')
		...
		with conn.cursor() as cur:
			for file_p in dir_path.glob('**/*.log'):
				try:
					...
					try:
						logging.info("processing: %s", file_p)
						cur_process(molData,cur)
						logging.info("finished: %s", file_p)
						i=i+1
					except Exception:
						logging.info("%s: sql operate error", file_p.as_posix())
				except Exception:
					logging.info("%s: parse moldata error", file_p.as_posix())		
	end_time = datetime.now()
	elapsed_time = end_time - start_time
	elapsed_seconds = int(elapsed_time.total_seconds())
	elapsed_time_str = str(timedelta(seconds=elapsed_seconds))
	logging.info("程序结束时间: %s", end_time.strftime('%Y-%m-%d %H:%M:%S'))
	logging.info("程序耗时: %s ,插入次数: %s",elapsed_time_str,str(i))